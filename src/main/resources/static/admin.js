    ...
    fetch('http://localhost:8080/api/update', {
        ...
        .then(response => {
            $('#' + id).replaceWith('<tr id=' + id + '>' +
                ...
                '<td> <' +
                '' +
                'button type="button" onclick="getModalDelete(' + id + ')" class="btn btn-danger btn-sm">Delete</button> </td>' +
                '</tr>');
        });
}
...
function showHeader() {
    fetch('http://localhost:8080/api/getUser')
        ...
        .then(user => {
            ...
            let rolesList = document.createElement('ul2');
            for (let i = 0; i < user.roles.length; i++) {
                let role = document.createElement('li');
                role.textContent = user.roles[i].roleName + " ";
                rolesList.appendChild(role);
